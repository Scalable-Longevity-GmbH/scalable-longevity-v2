...
export default function ResultsPreview({
  ...
  const gaugeT = Math.max(0, Math.min(1, (delta + RANGE) / (2 * RANGE)));
  ...
  const segIndex = Math.min(2, Math.floor((1 - gaugeT) * 3));

  // indicator position along arc: t=0 (right) -> 0, t=1 (left) -> pi
  const theta = Math.PI * gaugeT;
